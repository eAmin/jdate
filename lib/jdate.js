 ...
 * Copyright (C) 2003,2008 Behdad Esfahbod
 */

//TODO: imeplement setTimezone method to set local timezone.
...
var JDate = function(datetime, utctime) {
	...
	jdate._format = function(input, datee) {
		var pattern = /yy(?:yy)?|M{1,4}|d{1,4}|([HhmsTt])\1?|[LlZ]/g,
		ampm = function(a) { 
			return (!!a && a !== null) ? Object.keys(locales.ampm) : Object.values(locales.ampm);
		},
		replacement = {
			yyyy: datee[0],
			yy:   (datee[0] + '').slice(2),
			...
			TT:   hour < 12 ? ampm()[0] : ampm()[1],
			T:    hour < 12 ? ampm()[0] : ampm()[1],
			tt:   hour < 12 ? ampm(true)[0] : ampm(true)[1],
			t:    hour < 12 ? ampm(true)[0] : ampm(true)[1],
...
Object.values = function(object) {
	var values = [];
	for (var key in object) {
		hasOwnProperty.call(object, key) && values.push(object[key]);
	}
